...
// Custom Tab Types
type ViewType = 'dashboard' | 'ai' | 'calendar' | 'profile' | 'edit_profile' | 'add_crop' | 'add_task';
...
export default function FosholApp() {
  ...
  return (
    <div className={`flex justify-center min-h-screen bg-stone-200 dark:bg-stone-950 sm:p-4 md:p-6 lg:p-8 items-center font-sans transition-colors duration-300`}>
      <div className="w-full max-w-2xl bg-stone-50 dark:bg-stone-900 shadow-2xl sm:rounded-3xl overflow-hidden flex flex-col h-[100dvh] sm:h-[850px] relative transition-colors duration-300">
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden relative scroll-smooth bg-stone-50 dark:bg-stone-900 pb-28">
          <AnimatePresence mode="wait" initial={false}>
            {activeView === 'dashboard' && <DashboardView key="dashboard" tasks={tasks} toggleTask={toggleTask} />}
...
function DashboardView({ tasks, toggleTask }: { tasks: any[], toggleTask: (id: number) => void }) {
  return (
    <motion.div 
    ...
    >
      ...
      <section>
        <div className="flex justify-between items-end mb-4 px-1">
          ...
          <button className="text-sm text-emerald-600 dark:text-emerald-400 font-bold">View All</button>
        </div>
